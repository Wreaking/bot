...
class CommandLoader {
    ...
    validateCommand(command, filePath) {
        ...
        if (!command.data.name) {
            return { valid: false, reason: 'Missing command.data.name' };
        }

        if (!command.data.description) {
            return { valid: false, reason: 'Missing command.data.description' };
        }

        // Check for SlashCommandBuilder methods
        try {
            if (typeof command.data.toJSON !== 'function') {
                return { valid: false, reason: 'Command data is not a SlashCommandBuilder instance' };
            }
            
            // Test serialization
            const serialized = command.data.toJSON();
            if (!serialized.name || !serialized.description) {
                return { valid: false, reason: 'Command data serialization failed' };
            }
        } catch (error) {
